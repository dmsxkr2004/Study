...
parameters = [
    {"C":[1, 10, 100, 500, 1000], "kernel":["linear"], "degree":[3,4,5,6,7]},    # 12 [1, 10, 100, 1000]<- 5 x [3,4,5] <-5 = 25 # 인덱스 갯수 따라 숫자 곱
    {"C":[1, 10, 100, 1000], "kernel":["rbf"], "gamma":[0.001,0.001,0.0001]},       # 6  # 4x3 = 12
    {"C":[1, 10, 100, 500, 1000], "kernel":["sigmoid"],
     "gamma":[0.01, 0.001, 0.0001], "degree":[3,4,5,6]}                     # 24 5 x 3 x 4 = 60
]                                                                       # 총 42개 12 + 6 + 24 = 42
...
model = HalvingGridSearchCV(SVC(), parameters, cv=kfold, verbose=1, # CV = cross validation
                     refit = True, n_jobs=-1)
#3. 훈련
import time
start = time.time()

...
w1 = tf.compat.v1.Variable(tf.compat.v1.random.normal([2,3]), name='weight')
b1 = tf.compat.v1.Variable(tf.compat.v1.random.normal([3]), name = 'bias')
...
for i in range(10001):
    loss_val,hy_val,b_val,_ = sess.run([loss, hypothesis, [b1, b2], train], feed_dict = {x : x_data, y : y_data})
    if i % 200 == 0:

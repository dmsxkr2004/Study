...
#                                                    인풋은 (13, 5)
#                   단어사전의 개수                   단어수, 길이
model.add(Embedding(input_dim = 50, output_dim = 50, input_length = 5))
...
token2.fit_on_texts(x_predict)
print(token2.word_index)
y_test = token2.texts_to_sequences(x_predict)
y_test = pad_sequences(y_test, padding = 'pre', maxlen=5)
...
# print(y_pred)
if(y_pred<=0.5):
    print("결과 : ", y_pred*100,"%확률로 부정입니다")
elif(y_pred>0.5):
    print("결과 : ", y_pred*100,"%확률로 긍정입니다")
